...
function getDb(): Promise<IDBDatabase> {
  if (!dbPromise) {
    dbPromise = new Promise((resolve, reject) => {
      const req = indexedDB.open('geo-textures', 1)
...
const colorsKey = (id: string) => `geo_colors_${id}`
const vegKey    = (id: string) => `geo_veg_${id}`
const annKey    = (id: string) => `geo_annotations_${id}`
const orgKey    = (id: string) => `geo_org_${id}`
const viewsKey  = (id: string) => `geo_views_${id}`
...
export const localViewerAdapter: ViewerAdapter = {
  ...
  async fetchMaterials() {
    return read<MaterialDef[]>('geo_materials', [])
  },

  async saveMaterial(material) {
    const rest = read<MaterialDef[]>('geo_materials', []).filter(m => m.id !== material.id)
    write('geo_materials', [...rest, material])
  },

  async deleteMaterial(materialId) {
    write('geo_materials', read<MaterialDef[]>('geo_materials', []).filter(m => m.id !== materialId))
